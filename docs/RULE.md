# Master–Style Clean Code Checklist

Based on Master's blog posts, videos, and guides, we distilled the following **strict rules** for organizing and naming code. These checkpoints reflect Master's style for maintainable, scalable Python projects:
...
- **Dependency injection and configuration.** Pass dependencies (config, database connections, HTTP clients) into services rather than hard-coding them. Master recommends using Python DI (or FastAPI's built-in DI) to keep components loosely coupled【27†L183-L191】【33†L1-L4】. Manage configuration centrally (e.g. environment variables in a `.env` file and a `config.py` module). Include a `.python-version` file or similar to pin the Python version【33†L1-L4】.  
- **Folder and file structure (example):** Adopt a standard layout such as: 
  ...
  └── LICENSE.md               # license file (Master: "non-negotiable")【21†L153-L160】
  ```  
  This follows Master's advice to isolate source code in `src/`, keep tests and docs separate, and include standard files【19†L1-L4】【21†L92-L100】【21†L109-L118】.  
...
- **Logging and monitoring.** Centralize logging (e.g. configure a logger in one module). Master stresses having "centralized logging" for debugging large apps【33†L1-L4】. Don't scatter logging calls everywhere; use a common logger configuration.  
- **Avoid premature abstraction.** Use design patterns and abstractions only when they solve a concrete problem【12†L69-L77】【12†L99-L107】. Master warns against "overengineering": don't apply patterns just for the sake of it. Start simple, and refactor only when needed.  
...
- **Code readability.** Emphasize type hints, clear naming, and "an intuitive interface" for your APIs and classes【27†L167-L176】. Write code that reads easily – Master often says "code is read more than written." Aim for code that describes itself, with consistent conventions throughout.  
